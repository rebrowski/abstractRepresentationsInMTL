...
Analyses = {'CategoriesFromMeanResponse', ...
            'StimuliFromTrials', 'CategoriesFromTrials'};
...
whichUnits = select_units{u};
whichSubjOrSess = per_sub_or_sess{sors};

fontSize = 6;

outfn = ['decoding_' whichAnalysis, '_' , whichUnits, 'units_per', ...
         whichSubjOrSess, '.png'];


%% load data for requested analyses:
if strcmp(whichAnalysis, 'CategoriesFromTrials')
    infn =  ['classification_results_categories_fromtrials_per' ...
             whichSubjOrSess '_' whichUnits 'units.mat'];
    load(infn);
...
if strcmp(whichAnalysis, 'StimuliFromTrials')
    infn =  ['classification_results_stimuli_per' whichSubjOrSess '_' whichUnits 'units.mat'];
    load(infn);
...
if strcmp(whichAnalysis, 'CategoriesFromMeanResponse')
    infn = ['classification_results_categories_frommeanresponse_per' ...
            whichSubjOrSess '_' whichUnits 'units.mat'];
    load (infn)
    guessrate = 100/numel(ulabels); % percent
    plotCategories = true;
