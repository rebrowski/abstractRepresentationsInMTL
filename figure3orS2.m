...
%% switch whether we want to look at all data or just data from the
%% first session recorded in each patient (if there are multiple)
% if set to 'firstSessionPerPatient' -> Figure S2, otherwise: Figure3

whichUnits = 'firstSessionPerPatient' 
%whichUnits = 'all'
...
%% select units to plot (see also right at the top)
if strcmp(whichUnits, 'firstSessionPerPatient')

    subjidx = zeros(1,numel(cluster_lookup.subjid));
    usubs = unique(cluster_lookup.subjid);
    for s = 1:numel(usubs)
        idx_ = cluster_lookup.subjid == usubs(s);
        firstsess = min(cluster_lookup.sessid(idx_));
        disp(sprintf('subj %d, firstsess: %d', usubs(s), firstsess));
        subjidx(cluster_lookup.sessid == firstsess) = 1;
    end
    subjidx = subjidx';
    fname = 'figureS2.tiff'
    
else
    subjidx = ones(1,numel(cluster_lookup.subjid))';
    fname = 'figure3.tiff'
end
...
for r = 1:numel(regions)
    ...
    % if we look at just the first session of each patient:
    idx = idx & subjidx;
...
for r = 1:numel(regions)
    ...
    plot_mds(mdsax(r), zvals(idx,:), [], catticks,false,category_colors, markSize);
    if r == 1
        xlabel(mdsax(r), 'D1 (1-R)');
        ylabel(mdsax(r), 'D2 (1-R)');
    end
    set(mdsax(r), 'FontSize', fontSize)
...
for r = 1:numel(regions)
    ...
    add_text_topleft(dendax(r),annot(aidx(ac)), 0.02, 0.03, fontSize, 'bold');
    ac = ac + 1;
end

%% write figure to file
print(figh, fname,'-dtiff','-r600');
